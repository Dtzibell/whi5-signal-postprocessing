...
for path in path_to_files:
    ...
    # else:
        ...
        if min(x_time) <= starvation_start < max(x_time) and len(y_signal)>10 and max(x_time) >= (starvation_end+50):
            # in words: does not go any further if cell is born after start of starvation,
            # disappears before start of starvation or is only on the screen for one frame # AP - increase to 10, add existence after starvation condition (ideally we want at least 1 cycle in recovery)
            ...
            #GRAPH
            plt.plot(x_time, y_signal_scaled, color ="r")
